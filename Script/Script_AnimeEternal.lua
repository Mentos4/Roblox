...
if getgenv and getgenv().DexUnload then
	pcall(getgenv().DexUnload)
end
...
Main = (function()
	...
	Main.SecuredGuis = Main.SecuredGuis or {}
	Main.SecureGui = function(gui)
		table.insert(Main.SecuredGuis, gui)
...
Main.CreateMainGui = function()
		local gui = create({
			...
			{22,"TextButton",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="CloseDex",Parent={6},Position=UDim2.new(1,-72,0,0),Size=UDim2.new(0,24,1,0),Text="X",TextColor3=Color3.new(1,0.2,0.2),TextSize=16,}},
		})
		Main.MainGui = gui
...
openButton.MainFrame.Visible = false
		...
		local closeDexBtn = gui.OpenButton.MainFrame.BottomFrame.CloseDex
		
		local function unloadDex()
			if Main.SecuredGuis then
				for _, guiItem in pairs(Main.SecuredGuis) do
					pcall(function() guiItem:Destroy() end)
				end
			end
			if getgenv then getgenv().DexUnload = nil end
		end
		
		closeDexBtn.MouseButton1Click:Connect(unloadDex)
		if getgenv then getgenv().DexUnload = unloadDex end
